  ...
  try {
    ...
    if (!res.ok) {
      ...
      const criticalTables = {
        ...
        couple_vendors:   ['id','couple_id','name','category','phone','quoted_total','status','events','source','profile_incomplete','notes','created_at','updated_at'],
...
const TDW_COUPLE_TOOLS = [
  ...
  },
  {
    ...
    description: "Update a vendor's status in the couple's vendor list. When booking (status=booked), optionally provide quoted_price and advance to automatically create expense entries — advance becomes a paid expense, balance becomes a committed expense. When marking paid (status=paid), the vendor and their expense are both marked paid.",
    input_schema: {
      type: 'object',
      properties: {
        ...
        quoted_price: { type: 'number', description: 'Total quoted price in rupees (optional — for booking only)' },
        advance: { type: 'number', description: 'Advance amount paid in rupees (optional — for booking only)' },
        event: { type: 'string', description: 'Event name this vendor is booked for (optional)' },
      },
    ...
    switch (toolName) {
      ...
      case 'save_to_muse': {
        ...
        if (ogImage) {
          ...
          return "Link saved to your Muse board, but I could not generate a preview for this site. For the best experience, take a screenshot and send it to me - I will save that image directly.";
        }
      }
      ...
      case 'update_vendor_status': {
        const { vendor_name, status, quoted_price, advance, event: vendorEvent } = toolInput;
        const coupleId = vendor && vendor.id ? vendor.id : userId;
        const { data: rows } = await supabase.from('couple_vendors')
          .select('id, name, quoted_total').eq('couple_id', coupleId).ilike('name', '%' + vendor_name + '%').limit(1);
        if (!rows || rows.length === 0) return "Vendor " + vendor_name + " not found in your list. Add them first.";
        const vendorRow = rows[0];

        // Update vendor status
        const vendorUpdate = { status: status, updated_at: new Date().toISOString() };
        if (quoted_price) vendorUpdate.quoted_total = quoted_price;
        await supabase.from('couple_vendors').update(safePayload('couple_vendors', vendorUpdate)).eq('id', vendorRow.id);

        // Scenario A/B/C expense logic — only when booking
        if (status === 'booked') {
          const qp = quoted_price || vendorRow.quoted_total || 0;
          const adv = advance || 0;
          const eventName = vendorEvent || 'general';
          const expenseMessages = [];

          if (qp > 0 && adv > 0) {
            // Scenario A: quoted price + advance — advance = paid, balance = committed
            const balance = qp - adv;
            await supabase.from('couple_expenses').insert([safePayload('couple_expenses', {
              couple_id: coupleId,
              vendor_name: vendorRow.name,
              description: vendorRow.name + ' — advance',
              actual_amount: adv,
              category: 'other',
              payment_status: 'paid',
              event: eventName,
            ...
            if (balance > 0) {
              await supabase.from('couple_expenses').insert([safePayload('couple_expenses', {
                couple_id: coupleId,
                vendor_name: vendorRow.name,
                description: vendorRow.name + ' — balance due',
              ...
              expenseMessages.push('Advance of ₹' + adv.toLocaleString('en-IN') + ' logged as paid. Balance of ₹' + balance.toLocaleString('en-IN') + ' logged as committed.');
            } else {
              expenseMessages.push('Advance of ₹' + adv.toLocaleString('en-IN') + ' logged as paid.');
            }
          } else if (qp > 0 && adv === 0) {
            // Scenario B: quoted price, no advance — full amount = committed
            await supabase.from('couple_expenses').insert([safePayload('couple_expenses', {
              couple_id: coupleId,
              vendor_name: vendorRow.name,
              description: vendorRow.name + ' — full quoted amount',
              actual_amount: qp,
              category: 'other',
              payment_status: 'committed',
              event: eventName,
            ...
            expenseMessages.push('₹' + qp.toLocaleString('en-IN') + ' logged as committed.');
          }
          // Scenario C: no quoted price, no advance — silent skip

          const expenseNote = expenseMessages.length > 0 ? ' ' + expenseMessages[0] : '';
          return vendorRow.name + ' locked as BOOKED.' + expenseNote + ' Money tab updated.';
        }

        // When marking paid — mark vendor paid and mark their committed expense as paid
        if (status === 'paid') {
          await supabase.from('couple_expenses')
            .update(safePayload('couple_expenses', { payment_status: 'paid' }))
            .eq('couple_id', coupleId)
            .eq('vendor_name', vendorRow.name)
            .eq('payment_status', 'committed');
          return vendorRow.name + ' marked as PAID. Expense updated in Money tab.';
        }

        return vendorRow.name + ' updated to ' + status + '.';
      }
...
app.post('/api/whatsapp/incoming', async (req, res) => {
  ...
  try {
    ...
    if (couple) {
      ...
      const coupleName = profile?.name?.split(' ')[0] || 'there';
      ...
      // ── Hard keyword overrides (no AI needed) ──
      const isExplicitMuse = bodyLower.includes('muse') || bodyLower.includes('inspo') || bodyLower.includes('inspiration') || bodyLower.includes('save to board');
      const isExplicitReceipt = bodyLower.includes('receipt') || bodyLower.includes('expense') || bodyLower.includes('paid') || bodyLower.includes('payment') || bodyLower.includes('bill');
      const isGuestIntent = bodyLower.includes('import') || bodyLower.includes('contact') || bodyLower.includes('guest');
      const urlMatch = body.match(/https?:\/\/[^\s]+/);
      const isSocialUrl = urlMatch && /instagram|pinterest|behance|wedmegood|vogue|brides|wedding|shaadi|style|look|decor/i.test(urlMatch[0]);

      // ── Guest import ──
      if (isGuestIntent && numMedia > 0) {
        // Handled above in vCard branch — shouldn't reach here
        return;
      }
      if (isGuestIntent && numMedia === 0) {
        await sendWhatsApp(fromPhone, `Hi ${coupleName}! To add guests, forward me their contacts from WhatsApp:
...
Open TDW → Muse to see it.`);
        ...
        if (urlMatch) {
          ...
          if (ogImg) {
            ...
            await sendWhatsApp(fromPhone, "Saved the link, but I could not get a preview for this site. Take a screenshot and send it here for a proper image on your board!");
          }
          return;
        }
      }
      ...
      if (isSocialUrl && !isExplicitReceipt) {
        ...
        if (ogImg) {
          ...
          await sendWhatsApp(fromPhone, "Saved the link to your Muse board, but I could not generate a preview for this site. For better results, take a screenshot and send it here - I will save that image directly!");
        }
        return;
      }

      // ── Image sent — use Haiku to classify ──
      if (imageUrls.length > 0 && apiKey) {
        try {
          const classifyRes = await fetch('https://api.anthropic.com/v1/messages', {
            ...
            body: JSON.stringify({
              ...
              messages: [{ role: 'user', content: [
                ...
                { type: 'text', text: 'Classify this image in one word: RECEIPT (if it shows a bill, invoice, payment receipt, price quote, or financial document) or MUSE (if it shows fashion, decor, wedding inspiration, venue, photography, makeup, or anything visually inspirational). Reply with only one word.' }
              ]}]
            })
          });
          ...
          if (classification === 'RECEIPT' || isExplicitReceipt) {
            // OCR the receipt
            const ocrRes = await fetch('https://api.anthropic.com/v1/messages', {
              ...
              body: JSON.stringify({
                ...
                messages: [{ role: 'user', content: [
                  { type: 'image', source: { type: 'url', url: imageUrls[0] } },
                  { type: 'text', text: 'Extract from this receipt. Return ONLY JSON: {"vendor_name":"...","total_amount":number,"date":"YYYY-MM-DD or null","category":"Photography/Decor/Catering/MUA/Attire/Venue/Entertainment/Other","notes":"what was paid for"}' }
                ]}]
              })
            });
            const ocrData = await ocrRes.json();
            let parsed = {};
            try { parsed = JSON.parse((ocrData.content?.[0]?.text || '').replace(/```json|```/g, '').trim()); } catch {}

            const vendorName = parsed.vendor_name || 'Unknown vendor';
            const amount = parsed.total_amount ? `₹${Number(parsed.total_amount).toLocaleString('en-IN')}` : 'unknown amount';
            const notes = parsed.notes || '';

            // Store pending OCR in a temp table or just ask for confirmation via simple reply tracking
            // For now: confirm and auto-log
            await supabase.from('couple_expenses').insert([{
              couple_id: couple.id,
              vendor_name: vendorName,
              description: notes || 'Receipt scanned via WhatsApp',
              actual_amount: parsed.total_amount || 0,
              category: parsed.category || 'Other',
              payment_status: 'paid',
              event: 'general',
            }]);

            const receiptMsg = '📋 Receipt logged!\n\n' + vendorName + ' — ' + amount + (notes ? '\n' + notes : '') + '\n\nLogged as paid in your Money tab. Open TDW → Plan → Money to review.\n\nWrong? Reply with the correct amount.';
            await sendWhatsApp(fromPhone, receiptMsg);
            return;
          } else {
            // MUSE classification
            for (const url of imageUrls) {
...
Open TDW → Muse to see it.`);
          ...
          console.error('[WhatsApp classify]', e.message);
          // Fallback — ask the couple
          await sendWhatsApp(fromPhone, `Got your image! Should I:

1️⃣ Save to your *Muse board* (inspiration)
2️⃣ Log as an *expense* (receipt/payment)
...
Reply 1 or 2.`);
      ...
      if (urlMatch && !isExplicitReceipt) {
        ...
        if (ogImg) {
          ...
          await sendWhatsApp(fromPhone, "Saved the link, but I could not get a preview for this site. For a proper image on your board, take a screenshot and send it here!");
        }
        return;
      }

      // ── Everything else — Full agentic DreamAi (WhatsApp parity) ──
      if (apiKey) {
        try {
...
app.get('/api/v2/couple/today/:userId', async (req, res) => {
  try {
    ...
    const [
      ...
      supabase.from('moodboard_items').select('id, vendor_id, image_url, title, created_at').eq('user_id', userId).order('created_at', { ascending: false }).limit(3),
...
// POST /api/v2/couple/muse — save a link or URL to muse board
...
app.post('/api/v2/admin/exploring-photos/upload', async (req, res) => {
  ...
  try {
    ...
    const { error: uploadError } = await supabase.storage
      .from('cover-photos')
      .upload(`exploring/${fileName}`, fileData, { contentType: mimeType, upsert: true });
    if (uploadError) throw uploadError;

    const { data: urlData } = supabase.storage.from('cover-photos').getPublicUrl(`exploring/${fileName}`);
...
async function fetchOgImage(url) {
  try {
    ...
    const res = await fetch(url, {
      signal: controller.signal,
      headers: { 'User-Agent': 'Mozilla/5.0 (compatible; TDWBot/1.0)' },
    });
...
// POST /api/v2/couple/receipt-scan — OCR a receipt image via Claude Haiku vision
...
${JSON.stringify(context || {}, null, 2)}`;
    ...
    if (image_base64) {
      messages.push({ role: 'user', content: [
        ...
        { type: 'text', text: message || 'What is in this image? If it looks like a receipt or invoice, use add_expense to log it. If it looks like wedding inspiration (decor, fashion, makeup, venue, photography), use save_to_muse to save it to my Muse board.' }
      ]});
    } else {
    ...
    while (iterations < MAX_ITERATIONS) {
      ...
      const toolResults = [];
      const QUERY_TOOLS = ['query_schedule', 'query_revenue', 'query_clients', 'general_reply', 'save_to_muse', 'query_budget', 'query_tasks', 'query_vendors'];
...
// P1.5: Morning briefing cron endpoint — push notification only, no WhatsApp
// Called by Railway cron at 7AM IST (1:30 AM UTC) daily
