...
app.post('/api/co-planner/invite', async (req, res) => {
  try {
    ...
    // First invite is always free regardless of tier (removes onboarding friction)
    const tierLabel = user.couple_tier === 'elite' ? 'platinum' : user.couple_tier === 'premium' ? 'gold' : 'basic';
    let tokenCost = 0;
    if (active.length > 0) {
      // Second invite onwards: free for platinum, 1 token for gold, 2 for basic
...
app.post('/api/enquiries/:id/messages', async (req, res) => {
  try {
    const { from_role, content: filteredContent, attachments } = req.body || {};
    ...
    // Apply contact filtering (Airbnb-style — silent replacement)
